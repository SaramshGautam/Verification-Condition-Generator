...
program
    : pre=assertion statementlist post=assertion
        ...
        $pre.tree.print();   System.out.print(" => "); $post.tree.print();  System.out.println();
...
boolexp returns [Exp tree]
    : t=boolterm
		{ $tree = $t.tree; }
    | boolterm '=>' boolterm
    { $tree = new OpExp($boolterm1.tree, OpExp.Op.IMPLY, $t.tree); }
    | boolterm '<=>' boolterm
    { $tree = new OpExp($boolterm1.tree, OpExp.Op.EQUIV, $t.tree); }
    ;
